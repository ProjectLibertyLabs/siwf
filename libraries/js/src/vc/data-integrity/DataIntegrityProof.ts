...
export class DataIntegrityProof extends LinkedDataProof {
  ...
  async verifySignature({ verifyData, verificationMethod, proof }: any): Promise<boolean> {
    ...
    if (!isSupportedAlgorithm) {
      ...
      const message = `The verifier's algorithm "${verifier.algorithm}" ` + `${messageSuffix}`;
...
function _processSignatureParams({ signer, requiredAlgorithm }: any): any {
  ...
  if (!isSupportedAlgorithm) {
    const supportedAlgorithms = Array.isArray(requiredAlgorithm) ? requiredAlgorithm.join(', ') : requiredAlgorithm;
    const messageSuffix = Array.isArray(requiredAlgorithm)
      ...
      : `does not match the required algorithm for the cryptosuite ` + `"${supportedAlgorithms}".`;
    const message = `The signer's algorithm "${signer.algorithm}" ` + `${messageSuffix}`;
    throw new Error(message);
  }
